...
[EditorTool("Platform Tool", typeof(RoadPlace))]
class PlatformTool : EditorTool
{
    ...
    {
        var Target = ((RoadPlace)target).transform;

        Target.transform.position = new Vector3(Mathf.RoundToInt(Target.position.x), ((int)(Target.position.y / 0.5f)) * 0.5f, Mathf.RoundToInt(Target.position.z));
